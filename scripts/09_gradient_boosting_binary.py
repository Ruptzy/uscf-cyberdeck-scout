...
def main():
    ...
    print("="*50)
    print("PHASE 7: GRADIENT BOOSTING BINARY MODEL")
    print("="*50)
    ...
    # 4. Hyperparameter Tuning using pure Validation Set
    ...
    # Manual loop evaluates explicitly against X_val to strictly protect the chronological time-split
    ...
    metrics_path = os.path.join(out_dir, 'gb_binary_two_ratings_metrics.csv')
    imp_path = os.path.join(out_dir, 'gb_binary_two_ratings_importances.csv')
    ...
    print("1. Best Logistic Regression:")
    print("   - Validation ROC-AUC: 0.6253 | F1: 0.5088")
    print("   - Test ROC-AUC:       0.5970 | F1: 0.4848")
    print("2. Random Forest:")
    print("   - Validation ROC-AUC: 0.6845 | F1: 0.6100")
    print("   - Test ROC-AUC:       0.6158 | F1: 0.5610")
    print(f"\n3. Gradient Boosting:")
    print(f"   - Validation ROC-AUC: {val_m[4]:.4f} | F1: {val_m[3]:.4f}")
    print(f"   - Test ROC-AUC:       {test_m[4]:.4f} | F1: {test_m[3]:.4f}")
    
    print("\nVerdict:")
    if test_m[4] > 0.6158 and test_m[3] > 0.5610:
        print("Gradient Boosting outperformed both Random Forest and Logistic Regression. By iteratively correcting errors, the boosting algorithm successfully found a tighter, more accurate boundary for player/opponent ratings, fully justifying the extra algorithmic complexity.")
    elif test_m[4] > 0.5970 and test_m[3] > 0.4848:
        print("Gradient Boosting outperformed Logistic Regression, but failed to meaningfully beat the Random Forest on the unseen test set. The models are hitting a strict performance ceiling. This proves that you cannot mathematically squeeze more accuracy out of these base ratings alone—the next step MUST be advanced feature engineering.")
    else:
        print("Gradient Boosting failed to meaningfully outperform the baselines. The model is either severely overfitting or mathematically stuck. This proves that you cannot squeeze more accuracy out of these base ratings alone—the next step MUST be advanced feature engineering.")
    print("="*50 + "\n")
