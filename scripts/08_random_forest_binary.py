...
def main():
    ...
    print("="*50)
    print("PHASE 6: RANDOM FOREST BINARY MODEL")
    print("="*50)
    ...
    # RF does not suffer from dummy variable trap, but drop_first maintains parallel structure
    ...
    metrics_path = os.path.join(out_dir, 'rf_binary_two_ratings_metrics.csv')
    imp_path = os.path.join(out_dir, 'rf_binary_two_ratings_importances.csv')
    ...
    print("\n>> Comparison to Best Logistic Baseline (Baseline 2) <<")
    print("Previous Logistic Metrics:")
    ...
    if test_m[4] > 0.5970 and test_m[3] > 0.4848:
        print("The Random Forest effectively outperformed Logistic Regression. The tree-based architecture successfully captured non-linear interactions (e.g. certain rating tiers where upset probabilities shift dramatically), justifying the added complexity of moving away from linear modeling.")
    else:
        print("The Random Forest did NOT meaningfully outperform Logistic Regression on the test set. This strongly suggests that with these base features, the core relationship is purely linear. To break this performance ceiling, future iterations will need advanced feature engineering (e.g. rolling win streaks, activity metrics) rather than just a more complex algorithm.")
    print("="*50 + "\n")
