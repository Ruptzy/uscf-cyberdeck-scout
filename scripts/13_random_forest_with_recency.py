...
def main():
    ...
    print("="*50)
    print("PHASE 11: RANDOM FOREST WITH RECENCY FEATURES")
    print("="*50)
    ...
    metrics_path = os.path.join(out_dir, 'rf_binary_with_recency_metrics.csv')
    imp_path = os.path.join(out_dir, 'rf_binary_with_recency_importances.csv')
    ...
    print("\n>> Comparison to Previous Best Model (Base RF) <<")
    print("Previous RF Metrics (No Recency):")
    print("- Validation ROC-AUC: 0.6845 | F1: 0.6100")
    print("- Test ROC-AUC:       0.6158 | F1: 0.5610")
    print(f"\nNew RF Metrics (With Recency):")
    ...
    if test_m[4] > 0.6158 and test_m[3] > 0.5610:
        print("The recency features successfully broke the performance ceiling! Providing the Random Forest with historical momentum and activity context improved the algorithm's generalization on the unseen test set, proving that static ratings alone are insufficient.")
    else:
        print("The recency features did NOT meaningfully outperform the basic rating features. This suggests that while momentum exists in chess, it is either already fully captured by the static rating, or the dataset is too small for the Random Forest to learn stable rules about historical activity.")
    print("="*50 + "\n")
