...
def main():
    ...
    max_players = config['collection'].get('max_players', 500)
    max_events_total = config['collection'].get('max_events_total', 2000)
    batch_target_usable_games = config['collection'].get('batch_target_usable_games', 1500)
    ...
    while queue:
        ...
        if events_fetched_run >= max_events_total:
            ...
            break
        if total_valid_games_parsed_run >= batch_target_usable_games:
            stop_reason = f"Batch Target Usable Games ({batch_target_usable_games}) Reached"
            break
        ...
        for ev in recent_events:
            ...
            if events_fetched_run >= max_events_total or total_valid_games_parsed_run >= batch_target_usable_games:
                break
    ...
    # 4. Final Summary
    print("\n" + "="*50)
    print("PHASE 13: BATCH SCALE COLLECTION SUMMARY")
    print("="*50)
    ...
    print(f"Usable Games Added (This Run): {total_valid_games_parsed_run}")
